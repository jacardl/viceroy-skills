...
# 直隶按察使 · 公众号发布技能
  ...
  **执行前必读**：写 HTML 前先用渲染脚本生成（CSS 已内嵌，无需手动读取 reference）：
  ...
  6. `python3 scripts/push.py [--html /tmp/article.html] [--skip-illustration] [--skip-cover] [--delete-first <draft_id>]` 推草稿
     - 配图：自动提取 H2 小节（最多 5 张）→ xiaohu-ip-studio 生成 → 注入 HTML → 上传 mmbiz
...
**陷阱A：「不是X是Y」是长程正则匹配，word-swap 修不完（ponytail 重坑，命中10处）**

验证脚本的检测 regex 是 `不是.+?[，,].+?是`（最大宽度匹配，从「不是」到文本中任意后续的「是」）。即使把「不是X而是Y」改成「没有X有Y」，中间若出现独立「是」字，仍会被捕获。
...
**陷阱D：破折号 —— 全部替换（ponytail 命中10处，2026-06-27）**
