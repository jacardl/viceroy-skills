...
- **抽象词换具体词要克制。** "细小琐碎的事情"换成"小红点"看似
  ...
  会卡住，否则保留作者自己的抽象层级。
- **从零写到 5000 字 vs 改稿：清单扫描范围不同。** 改稿时清单只
  扫你动过的地方（原文有 AI 套话不动）。但如果任务是"基于素材
  从零写到 5000 字长文"（如 `zhililong`），AI 写出的每一段都
  算"动过"，清单必须扫**全文**，不能跳过。这种场景清单扫描会
  命中大量内容，需要在产出过程中就内化约束，不要等写完再扫。
  实测经验：每节 800-1100 字的 5 节结构，未内化约束时清单命中
  5-10 处，内化后 0-2 处。
