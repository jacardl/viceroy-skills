...
Validation rules:
- Every analysis run must save two workspace files: `prompt-analysis.json` for categorized prompts/data support/rationale and `sources.json` for all searched sources.
- Final frontend JSON must include `artifacts.promptAnalysisPath` and `artifacts.sourcesPath` so the frontend can read those files.
...
- `references[].url` must be real and traceable. Each run should include at least 100 de-duplicated source URLs; omit unknown URLs instead of inventing.
