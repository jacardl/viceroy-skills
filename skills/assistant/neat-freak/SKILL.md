...
### 第零步：尺寸体检（防膨胀）→ 顺便检查 Skill 自身引用完整性

**任何同步动作之前，先 `wc -l` 关键文件，同时检查 Skill 自身的「参考资料」节引用的文件是否存在：**

| 检查项 | Soft limit | 超过/缺失怎么办 |
|--------|-----------|----------------|
| `CLAUDE.md` / `AGENTS.md` | ~300 行 / ~15KB | 先精简：扫顶部 blockquote / 历史叙事段 → 删 / 迁 docs |
| 单条 memory 文件 | ~100 行 | 拆 / 删 / 改成 reference |
| `docs/<single>.md` | ~1500 行 | 切分成多文件，加目录索引 |
| **已发现技能系统异常（2026-05-24）**：`zhiliGitHub` skill 的内部字数表错误地将 `zhiliComments` 标为 `4000-5000字`，正确值为 `800-1500字`。`zhilicomments` SKILL.md（social-media/）显示正确值，但 `skill_manage patch` 报 `not found` — 技能注册表损坏，无法直接修改。**凡字数相关问题，以 `zhiliComments` SKILL.md 和 `MEMORY.md` 为准，不引用 `zhiliGitHub` 内部字数表。**

**Skill 自身引用的 `references/*.md`** | — | **发现缺失立即创建，不跳过** |

> ⚠️ **本 Skill 自身就有过因「第一次从未被完整跑过」而引用文件缺失的记录（2026-05-21 发现 sync-matrix.md 和 agent-paths.md 在本次会话前不存在）。每次触发 neat-freak 时，第零步必须同时检查 `Skill目录/references/` 下的所有引用的文件是否真实存在。缺失则立即按参考资料节描述的规格创建，这是维护责任，不是可选步骤。**

**单文件 Skill 臃肿的标准处理路径（zhilicomments 432行案例）：**
1. 判断内容类型：格式规范 / API踩坑记录 / 创意方法论 / 触发条件+流程
2. 将非核心内容迁入 `references/<topic>.md`（如 `references/wechat-pitfalls.md`、`references/format.md`）
3. 主 SKILL.md 只保留：触发条件、流程摘要、"详见 references/xxx" 指针
4. 目标：主文件 ≤ ~150 行；references/ 下单文件 ≤ ~500 行
