...
- [ ] **3.5 与符号/变量表**：3.5 参数表「符号/变量」列是否与 3.4.1 或正文普通算式 **同形同义**
...
- [ ] 若源材料中已有结构代码、JSON/YAML schema、profile 示例、字段名、样例 ID，交底书是否直接采用或等价脱敏改写这些源示例？
- [ ] 是否存在 Agent 临时编造的另一套字段口径（如自造 profile JSON、agent schema、业务场景）与源材料不一致？若有须替换为源材料口径。
- [ ] 第三章、第五章、第六章中的对象名和字段名是否一致，且能追溯到扫描材料或用户指定文件？
...
- [ ] **系统框图与流程图**：均为 fenced mermaid，定稿已由**原始 mermaid 渲染结果**转 PNG 并嵌入 Word，**未手工重画导致与源码不一致**；`.md` 已交付，`.docx` 已生成或已按提示手动 `md_to_docx.py` 补全
