...
## 7.8 示例 / schema 准确性（必遵）

当项目材料、设计文档或用户指定文件中已经存在结构代码、JSON/YAML schema、profile 示例、流程示例、字段名或样例 ID 时：

1. **优先直接引用源材料**：交底书中的代码块和结构示例必须优先从源文件中摘取或等价改写，保持字段名、层级、样例 ID、枚举值和业务口径一致。
2. **禁止临时另造口径**：不得为了让交底书显得完整而自行编造另一套 `profile`、`agent`、`evidence_pool`、`product_context` 等 schema；如确需脱敏或压缩，须说明“字段取自源材料，已做脱敏/节选”。
3. **源路径自检**：生成正文前，确认每个关键示例至少能追溯到扫描材料中的具体文件或段落；若没有源材料，才可使用“示意性示例”，并在正文中明确为“可选实施例示意”。
4. **跨章一致**：第三章、第五章、第六章的字段名和对象名须一致，例如 `profile_skeleton`、`locked_internal_fields`、`profile_evidence_pool`、`enriched_profile`、`synthetic_user_agent` 不得混用不同命名。

## 7.9 Word 公式可读性（优先）

专利交底书面向代理人、业务方和发明人协作审查，**docx 可读性优先于 LaTeX 形式完整性**：

1. 对简单评分、阈值、乘加权重类公式，优先写成：**自然语言说明 + 普通算式文本 + 参数表**，例如“合并得分 = 产品相关度 × 证据置信度 × 无冲突系数”，再给出 `s_i = r_i × q_i × (1 - h_i)`。
2. 若使用 LaTeX，生成 Word 后必须检查：docx 中不得残留 `\[...\]`、`$$...$$`、`	ag{}`、`\cdot` 等源码；若出现，应改为普通算式或确保公式 PNG 正常嵌入。
3. 参数表中的符号列可用普通文本（如 `s_i`、`θ_s`、`L_p`），不强制使用 LaTeX；但须与正文算式同形同义。
4. 第六章参数示例也应使用可读表达，例如“合并得分 s_i = 0.9 × 0.8 × (1 - 0) = 0.72”，避免代理人打开 Word 后看到公式源码。
