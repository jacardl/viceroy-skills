...
- **图示定稿（Step 7）**：**3.2**/**3.4** 用 fenced **mermaid**；定稿 Word 必须嵌入由 **Markdown 中原始 mermaid 代码渲染后的图直接截屏** 得到的 PNG（优先用 Mermaid 官方渲染/浏览器截图，保留原始布局、分组、连线、中文标签等完整效果），不得用手工重画、简化图、重新排版图或 ASCII 图替代；执行方式、**`mmdc`** 安装、浏览器截图降级与规则见下表「交底书定稿交付」行及 **`tools/README.md`**。
- **示例与 schema 准确性**：若项目材料中已有结构代码、JSON/YAML schema、profile 示例、流程示例，交底书中的代码块必须优先直接取自源材料并保持字段名一致；不得为“看起来完整”临时编造不同口径的示例。
- **Word 公式可读性**：交底书面向代理人/业务方时，核心公式优先使用“自然语言 + 普通算式文本 + 参数表”的可读表达；只有用户明确需要或公式复杂到必须排版时才使用 LaTeX，并须确认 docx 中不会残留 `\[...\]`、`	ag{}` 等源码。
...
| 交底书定稿交付（**须同时** .md + .docx） | **3.2** 系统框图与 **3.4** 流程图均用 fenced ``mermaid``，**不要** ASCII 文字流程图/框图。定稿必须将 **Markdown 中原始 mermaid 渲染后的图直接截屏为 PNG** 后嵌入 Word；优先使用 Mermaid 官方渲染或浏览器打开包含原始 mermaid 的临时 HTML 后对渲染结果截图，以确保图的布局、分组、连线和标签完整；若 `mmdc` 失败，可用浏览器渲染原始 mermaid 后截图；**禁止手工重画、简化重排或用自制近似图替代**。执行 **`tools/mermaid_render.py`**：mermaid 转 PNG（失败块保留围栏）后默认生成同名 **.docx**；若 Word 失败，按 stderr 提示手动运行 **`md_to_docx.py`**。详见 **`tools/README.md`** |
...
□ 除用户明确跳过外，完成摘要预览
□ 脱敏、mermaid（定稿均已由 Markdown 中原始 mermaid 官方渲染/浏览器渲染后的图直接截屏为 PNG，并嵌入 docx；未手工重画、未简化重排、未用自制近似图替代）、章节引用符合 template_reference；项目源材料已有 schema/代码示例时，交底书示例已直接取自源材料并保持字段名一致；含公式时 **3.4.1 符号表、§7.7 体例**或“自然语言 + 普通算式”可读表达已满足，docx 无裸 LaTeX 源码；**已交付 .md 与 .docx**，且**文件名符合 §7.3 第 5 点**（**凡交付均含**时间戳后缀）；**正文无**技能/示例仓库类文末脚注
