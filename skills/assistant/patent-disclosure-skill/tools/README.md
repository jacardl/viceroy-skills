...
**生图失败降级**：某一围栏 `mmdc` 失败时**不中断**——该处**保留**原 `` ```mermaid`` … `` ``` `` 源码；其余块照常出图。仍写出定稿 `.md`，并**照常尝试**生成 Word（未出图块在 Word 中为 **Consolas 代码块**，与 `md_to_docx` 行为一致）。若 Word 必须显示图而 `mmdc` 不可用，推荐用浏览器/Obsidian/Markdown 预览直接渲染**原始 mermaid 源码**后截图为 PNG，并插入同样的 ``<!-- ![图示 n](mermaid_figures/...) -->`` 注释；**不要手工重画**，以免图与源码不一致。
...
**Word 版式**：**全部公式图**（行内与块级式 (1) 等）在 Word 中统一按约 **0.17 英寸**高度嵌入；**mermaid 框图/流程图**仍按 **5.5×8.2 英寸**上限等比嵌入。块级 PNG 默认与行内同字号（10.5pt）渲染，避免块级式显得过粗过大。对于简单评分/阈值类公式，优先在正文写普通算式文本，避免 Word 中出现裸 LaTeX 源码。
